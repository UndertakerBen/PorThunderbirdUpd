        ...
        static void Main()
        {
            ...
            if (File.Exists(applicationPath + "\\Thunderbird Beta x64\\Thunderbird.exe"))
            {
                ...
                if (!File.Exists(applicationPath + "\\Thunderbird Beta x64\\updates\\Profile.txt"))
                {
                    ...
                    String Arguments = File.ReadAllText(applicationPath + "\\Thunderbird Beta x64\\updates\\Profile.txt");
                    if (Arguments.Contains("-profile \"Thunderbird"))
                    {
                        ...
                        Process.Start(applicationPath + "\\Thunderbird Beta x64\\Thunderbird.exe", Arguments3 + sb.ToString());
                    }
                    ...
                    {
                        Process.Start(applicationPath + "\\Thunderbird Beta x64\\Thunderbird.exe", Arguments + sb.ToString());
                    }
                }
                else
                {
            ...
            {
                NewMethod(applicationPath, "Thunderbird Beta x64 ist nicht installiert");
            }
            else if (culture1.TwoLetterISOLanguageName == "ru")
            {
                NewMethod(applicationPath, "Mozilla Thunderbird Beta x64 не найден");
            }
            else
            {
                NewMethod(applicationPath, "Thunderbird Beta x64 is not installed");
            }
        }
        private static void NewMethod(string applicationPath, string message)
        {
            if (File.Exists(applicationPath + "\\Thunderbird Beta x64 Launcher.exe"))
            {
                FileVersionInfo launcherVersion = FileVersionInfo.GetVersionInfo(applicationPath + "\\Thunderbird Beta x64 Launcher.exe");
                MessageBox.Show(message + Environment.NewLine + "Launcher Version " + launcherVersion.FileVersion, "Thunderbird Beta x64 Launcher", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
            else
            {
                MessageBox.Show(message, "Thunderbird Beta x64 Launcher", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
    }
}
