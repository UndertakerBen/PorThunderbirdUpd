...
namespace Thunderbird_Launcher
{
    public partial class Form1 : Form 
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Thunderbird\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Thunderbird\updates\Profile.txt", "-no-remote -profile \"Thunderbird\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Thunderbird\updates\Profile.txt", "");
                this.Close();
            }
        }
