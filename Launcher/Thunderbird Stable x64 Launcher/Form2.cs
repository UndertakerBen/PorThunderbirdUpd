...
namespace Thunderbird_Stable_x64_Launcher
{
    public partial class Form2 : Form
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Thunderbird Stable x64\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Thunderbird Stable x64\updates\Profile.txt", "-no-remote -profile \"Thunderbird Stable x64\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Thunderbird Stable x64\updates\Profile.txt", "");
                this.Close();
            }
        }
