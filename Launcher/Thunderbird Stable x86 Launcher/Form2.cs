...
namespace Thunderbird_Stable_x86_Launcher
{
    public partial class Form2 : Form
    {
        ...
        private void Button1_Click(object sender, System.EventArgs e)
        {
            ...
            {
                File.WriteAllText(@"Thunderbird Stable x86\updates\Profile.txt", "-allow-downgrade -no-remote -profile \"profile\"");
                this.Close();
            }
            if (radioButton2.Checked)
            {
                File.WriteAllText(@"Thunderbird Stable x86\updates\Profile.txt", "-no-remote -profile \"Thunderbird Stable x86\\profile\"");
                this.Close();
            }
            if (radioButton3.Checked)
            {
                File.WriteAllText(@"Thunderbird Stable x86\updates\Profile.txt", "");
                this.Close();
            }
        }
